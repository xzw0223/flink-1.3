...
public abstract class RegisteredRpcConnection<
    ...
    public void start() {
        ...
        // 这个实用程序类实现了在另一个组件上注册一个组件的基础，例如在ResourceManager上注册TaskExecutor。
        // 这个{@code RetryingRegistration}实现了初始地址解析和返回重试策略。进入翻译页面

        // 创建注册对象
        final RetryingRegistration<F, G, S, R> newRegistration = createNewRegistration();

        if (REGISTRATION_UPDATER.compareAndSet(this, null, newRegistration)) {
            // 开始注册  注册成功后调用onRegistrationSuccess()        JobMaster 注册到 resourceManager
        ...
        future.whenCompleteAsync(
                (RetryingRegistration.RetryingRegistrationResult<G, S, R> result,
                    ...
                    if (failure != null) {
                        ...
                        // 如果注册成功,调用onRegistrationSuccess方法
                        if (result.isSuccess()) {
